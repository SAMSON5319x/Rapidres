...
def main():
    ...
    # Torch min/max bounds (needed for FGSM/PGD)
    ...
    rob_acc, rob_auc = TT.eval_under_attack(
        attack=args.attack,
        eps=args.eps,
    ...
    append_run(
        args.metrics_out,
        'torch_' + ('defended' if args.adv_train else 'baseline'),
        {
            'clean_acc': clean_acc,
            'clean_auc': clean_auc,
            'rob_acc': rob_acc,
    ...
    append_run(
        args.metrics_out,
        'lgb_baseline',
        {'clean_acc': acc_lgb, 'clean_auc': auc_lgb}
    )

    # Transfer attack (Torch → LightGBM)
    surr = MLP(in_dim=X_tr.shape[1], hidden=hidden)
    surr.load_state_dict(TT.model.state_dict())
    ST = SurrogateTransfer(surr)

    X_adv, clean_acc_lgb, adv_acc_lgb, tsr = ST.craft_and_transfer(
        LGB.model,
        X_va,
        y_va,
        attack=args.attack,
    ...
    append_run(
        ...
        'transfer_to_lgb',
        {
            'clean_acc': clean_acc_lgb,
            'adv_acc': adv_acc_lgb,
            'transfer_success': tsr,
            'attack': args.attack,
            'eps': args.eps
        }
    )
