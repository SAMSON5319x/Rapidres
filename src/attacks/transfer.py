...
class SurrogateTransfer:
    ...
    def craft_and_transfer(self, target_model, X, y, attack='fgsm', eps=0.1, steps=5, step_size=0.02):
        ...
        for xb, yb in ld:
            ...
            # simple per-batch bounds in standardized space
            lo = xb.min(dim=0).values
            hi = xb.max(dim=0).values

            if attack == 'fgsm':
                xadv = fgsm(self.s, xb, yb, eps, lo, hi)
            else:
                xadv = pgd(self.s, xb, yb, eps, step_size, steps, lo, hi)

            adv_list.append(xadv.detach().cpu().numpy())

        X_adv = np.concatenate(adv_list, axis=0)

        # Evaluate transferability on the target (LightGBM)
